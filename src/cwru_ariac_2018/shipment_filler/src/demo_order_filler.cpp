...
int main(int argc, char** argv) {
    ...
    ros::ServiceClient competition_client = nh.serviceClient<std_srvs::Trigger>("/ariac/start_competition");
    std_srvs::Trigger start_competition;
    while(!competition_client.call(start_competition)){
        ros::spinOnce();
    }
    ...
    ROS_INFO("done removing bad parts; attempt part relocations, as necessary");
    ...
    while (nparts_misplaced > 0) {
        ...
        ROS_INFO("INDEX_DES_PART: %u ", index_des_part);
    ...
    while (n_missing_parts > 0) {
        ...
        //specify place part:
        model_to_part(desired_models_wrt_world[n_missing_part], place_part, box_location_code);
    ...
    //repeat the whole inspection.
    //I tried to make the inspection a function but there were too many local variables and weird things with node handle
     boxInspector.compute_shipment_poses_wrt_world(g_order.shipments[0], box_pose_wrt_world, desired_models_wrt_world);
